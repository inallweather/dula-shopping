...
class LoginForm(FlaskForm):
    nickname = StringField('登录名', validators=[DataRequired(message='登录名不能为空'), Length(2, 15, message='登录名在5到15个字符之间')])
    password = PasswordField('密码', validators=[DataRequired(message='密码不能为空'), Length(3, 15, message='密码在5到15个字符之间')])
